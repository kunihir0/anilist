...
#[poise::command(slash_command, prefix_command, subcommands("set", "next", "vote"), guild_only)]
...
pub async fn next(ctx: Context<'_>) -> Result<(), Error> {
    ...
    if let Some(party) = settings.watch_party {
        match fetch_media_by_title(&data.http_client, &data.cache, &data.rate_limiter, &party.title).await {
            Ok(media) => {
                if let Some(next) = media.next_airing_episode {
                    ctx.say(format!(
                        "Next episode of **{}**:\n**Episode {}** airs in **{}**.",
                        party.title, next.episode, next.countdown()
                    )).await?;
                } else {
                    ctx.say(format!("No upcoming airing episodes found for **{}**.", party.title)).await?;
                }
            }
            Err(e) => {
        ...
        ctx.say("No watch party series is currently set. Use `/watch set <title>` first.").await?;
    }
    Ok(())
}
...
pub async fn vote(
    ...
    if let Some(o) = opt3 { options.push(o); }
    ...
    let embed = serenity::CreateEmbed::new()
        ...
        .description(format!("React with the corresponding number to vote for the next series:\n\n{}", description))
