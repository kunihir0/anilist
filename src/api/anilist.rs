...
impl From<AniListApiError> for AniListError {
    fn from(e: AniListApiError) -> Self {
        match e.status {
            ...
            _         => AniListError::Api { message: e.message, status: e.status },
        }
    }
}
    ...
    T: serde::de::DeserializeOwned,
{
    ...
    let resp = resp.error_for_status().map_err(|e| {
        if status_code == 429 {
            ...
            AniListError::Api { message: e.to_string(), status: Some(status_code) }
        }
    })?;

    let bytes = resp.bytes().await.map_err(|e| AniListError::Network(e.to_string()))?;
    ...
    let parsed: GraphQlResponse<T> = serde_json::from_slice(&bytes).map_err(|e| AniListError::Decode(e.to_string()))?;
...
pub async fn fetch_anime(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, search: &str) -> Result<Vec<Media>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
        format!("anime:{search}"),
...
pub async fn fetch_media_by_title(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, search: &str) -> Result<Media, Error> {
    let key = format!("media:title:{search}");
    if let Some(cached) = cache.get::<Media>(&key).await {
        return Ok(cached);
    }
    let data: MediaSearchData = graphql_post(client, rate_limiter, queries::ANIME_SEARCH_QUERY, json!({ "search": search })).await?;
    let media = data.page.media.into_iter().next().ok_or("Media not found")?;
    cache.set(key, &media).await;
    Ok(media)
}

pub async fn fetch_manga(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, search: &str) -> Result<Vec<Media>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
        format!("manga:{search}"),
...
pub async fn fetch_character(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, search: &str) -> Result<Character, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_studio(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, search: &str) -> Result<Studio, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_upcoming(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, season: &str, year: i32) -> Result<Vec<Media>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_airing(client: &Client, cache: &Cache, rate_limiter: &RateLimiter) -> Result<Vec<Media>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_random(client: &Client, rate_limiter: &RateLimiter, media_type: &str) -> Result<Media, Error> {
    let page = rand_page(100);
    let vars = json!({ "type": media_type, "page": page });
    let data: MediaSearchData = graphql_post(client, rate_limiter, queries::RANDOM_PAGE_QUERY, vars).await?;

    if let Some(media) = data.page.media.into_iter().next() {
        return Ok(media);
    }

    let fallback_data: MediaSearchData = graphql_post(client, rate_limiter, queries::RANDOM_PAGE_QUERY, json!({ "type": media_type, "page": 1 })).await?;
    fallback_data.page.media.into_iter().next().ok_or_else(|| "No results returned for random query.".into())
}
...
pub async fn fetch_user(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, username: &str) -> Result<AniListUser, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_staff(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, search: &str) -> Result<Staff, Error> {
    fetch_cached(
        client, cache, rate_limiter,
        format!("staff:{search}"),
        queries::STAFF_QUERY,
...
pub async fn fetch_recommendations(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, search: &str) -> Result<MediaRecommendationInfo, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_trending(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, media_type: &str) -> Result<Vec<Media>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_genre(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, genre: &str, media_type: &str) -> Result<Vec<Media>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_favourites(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, username: &str) -> Result<UserFavourites, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_staff_birthdays(client: &Client, cache: &Cache, rate_limiter: &RateLimiter) -> Result<Vec<StaffBirthday>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_watchlist(client: &Client, cache: &Cache, rate_limiter: &RateLimiter, username: &str, media_type: &str) -> Result<MediaListCollection, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_genres(client: &Client, cache: &Cache, rate_limiter: &RateLimiter) -> Result<Vec<String>, Error> {
    fetch_cached(
        client, cache, rate_limiter,
...
pub async fn fetch_filtered_media(
    ...
    let data: MediaSearchData = graphql_post(client, rate_limiter, queries::FILTER_QUERY, vars).await?;
    Ok(data.page.media)
}
