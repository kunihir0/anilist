...
use crate::api::anilist::{
    fetch_random, fetch_airing, fetch_trending, fetch_upcoming, fetch_staff_birthdays
};
use crate::utils::embeds::{
    media_embed, airing_page_embed, media_list_embed, upcoming_page_embed, staff_birthday_embed
};
...
pub async fn spawn_scheduler(ctx: serenity::Context, data: Arc<Data>) {
    ...
    for guild_schedules in schedules.values() {
        for entry in guild_schedules {
            if entry.active {
                if let Err(e) = register_job(&scheduler, entry.clone(), &ctx, &data).await {
